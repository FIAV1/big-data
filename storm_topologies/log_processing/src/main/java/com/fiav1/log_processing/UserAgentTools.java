...
public class UserAgentTools {
		...
		while (a_position < a_userAgent.length()) {
			...
			switch (status) {
				...
				case 1: // <SPAN class="codecomment"> Version number in progress</span>
					...
					res.append(c);
					break;
				case 2: // <SPAN class="codecomment"> Space encountered - Might need to end the
								// parsing</span>
	...
	public String[] getBotName(String userAgent) {
		...
		// <SPAN class="codecomment"> The following two bots don't have any version
		// number in their User-Agent strings.</span>
	...
	public String[] getOS(String userAgent) {
		...
		if ((pos = userAgent.indexOf("Windows-NT")) > -1) {
			...
			// <SPAN class="codecomment"> The different versions of Windows NT are decoded
			// in the verbosity level 2</span>
	...
	public String[] getBrowser(String userAgent) {
		...
		if ((pos = userAgent.indexOf("Lotus-Notes/")) > -1) {
			...
			if ((pos = userAgent.indexOf("MSIE 6.0")) > -1) {
				...
				res = getArray("MSIE", "<B>MSIE?</B>",
						"<B>MSIE?" + getVersionNumber(userAgent, userAgent.indexOf("MSIE") + 4) + "</B>");
